...
def call(call_args, cwd='.', shell=False, env=None):
    if args.quiet:
        ...
        print(call_args)
        subprocess.call(call_args, stderr=subprocess.STDOUT, cwd=cwd, shell=shell, env=env)
...
call(['python2', 'tools/git-sync-deps'], cwd='skia')
...
def build_ninja():
    ...
    if all_modules:
        call_args.extend(
            ['particles', 'skottie', 'skparagraph', 'skshaper', 'svg', 'skunicode'])

    call(' '.join(call_args), cwd='skia', shell=True)
